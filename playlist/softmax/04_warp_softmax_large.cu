...
/*
    ...
    Host to Device Transfer: 20.3596 ms
    Kernel Execution: 80.9056 ms
    Device to Host Transfer: 25.8515 ms
    Exit status: 0
...
int main() {
    ...
    int BLOCKS = (M + BLOCK_SIZE - 1) / BLOCK_SIZE;  // FIXED: Use BLOCKS for launch
    ...
    shuffle_softmax<<<BLOCKS, BLOCK_SIZE, SMEM_SIZE>>>(d_A, d_B, M, N);  // FIXED: Use BLOCKS not GRID_SIZE
